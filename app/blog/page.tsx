        ...
        <>
            ...
            <main className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
                <SectionReveal className="mb-10">
                    ...
                    <p className="mt-5 max-w-3xl text-lg leading-8 text-muted">Updates on climate justice, economic divestment, and community-led transformation across Nigeria. Curated insights from CoSET&apos;s network of advocates and researchers.</p>
                </SectionReveal>
                ...
                <div className="grid gap-8 lg:grid-cols-[1.2fr_0.8fr] xl:grid-cols-[1fr_300px]">
                    <div className="space-y-8 xl:col-span-1">
                        <SectionReveal className="grid overflow-hidden rounded-[2rem] border border-line bg-panel shadow-editorial md:grid-cols-[1.1fr_1fr]">
                            <Link href={`/blog/${featured.slug}`} className="relative min-h-[360px] group overflow-hidden">
                                <Image src={featured.image} alt={featured.title} fill sizes="(max-width: 768px) 100vw, 60vw" className="object-cover transition duration-500 group-hover:scale-105" />
                            </Link>
                            <div className="p-8">
                                ...
                                <Link href={`/blog/${featured.slug}`}>
                                    <h2 className="mt-4 font-display text-4xl font-extrabold leading-tight tracking-[-0.05em] text-navy hover:text-ember transition">
                                        {featured.title}
                                    </h2>
                                </Link>
                        ...
                        <div className="grid gap-8 md:grid-cols-2">
                            {rest.map((post, index) => (
                                <SectionReveal key={post.title} delay={index * 0.06} className="overflow-hidden rounded-[2rem] border border-line bg-panel shadow-soft">
                                    <Link href={`/blog/${post.slug}`} className="relative aspect-[16/10] block group overflow-hidden">
                                    ...
                                    <div className="p-6">
                                        ...
                                        <Link href={`/blog/${post.slug}`}>
                                            <h3 className="mt-3 font-display text-2xl font-bold tracking-[-0.04em] text-navy hover:text-ember transition">
                                                {post.title}
                                            </h3>
                                        </Link>
                    ...
                    <SectionReveal className="space-y-6 xl:col-span-1">
                        <div className="rounded-[2rem] bg-ink p-6 text-white shadow-editorial">
                            ...
                            <p className="mt-3 text-sm leading-7 text-white/70">Weekly briefings on climate justice, policy reform, and socio-ecological transformation.</p>
                        ...
                        <div className="rounded-[2rem] border border-line bg-panel p-6 shadow-soft">
                            <p className="font-display text-lg font-bold text-navy">Research Topics</p>
                            <div className="mt-4 flex flex-wrap gap-2">
                                {['Gas Flaring', 'Just Transition', 'Economic Divestment', 'Climate Loss & Damage', 'Trade Union Rights'].map((topic) => (
