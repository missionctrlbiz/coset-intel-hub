...
For scraping, the AI is tasked with extracting the *core report body* while stripping navigation and ads, returning it as clean semantic HTML.

```typescript
// Prompt instruction for scraping
purpose === 'web-scraping' ? `Extract core content as semantic HTML (h1-3, p, ul, table)` : `Extract metadata only`;
```
